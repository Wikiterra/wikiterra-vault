...
Wikiterra es una web estática, lo que significa que no hay lado servidor todo se ejecuta en lado cliente (en el navegador web, "chrome", "firefox", etc). Los artículos están escritos en formato [[markdown]], un lenguaje de marcado sencillo y fácil de leer, que mediante un [generador de sitios estáticos (SSG)](https://en.wikipedia.org/wiki/Static_site_generator) se convierte al formato web (HTML, CSS, JS) usando un 
